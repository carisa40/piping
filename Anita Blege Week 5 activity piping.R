...
write.csv(mysample,"~/Downloads/R Projects - ANA 500\\mysample_week5.csv",row.names = FALSE)
...
piped_sample<-mysample
  arranged_pipe<-arrange(piped_sample, airline)
  piped_sample<-filter(piped_sample, incidents_85_99<25)
  piped_sample<-rename(piped_sample, seats_airline = avail_seat_km_per_week)
  seats_airline<-select(piped_sample, incidents_00_14, incidents_85_99)
  summary_piped<-summary(seats_airline)
  print(summary_piped)
  
piped_sample<-mysample %>%
  arrange(airline) %>%
  filter(incidents_85_99<25) %>%
  rename(seats_airline = avail_seat_km_per_week)%>%
